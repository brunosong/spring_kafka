...
import com.example.batchlisten.common.Foo1;
...
public class Controller {
    ...
    /* 트랜잭션 FAIL 테스트 */
    @PostMapping(path = "/send/foos/fail/{what}")
    public void sendFooFail(@PathVariable String what) {
        this.template.executeInTransaction(kafkaTemplate -> {
            StringUtils.commaDelimitedListToSet(what).stream()
                    .map(s -> new Foo1(s))
                    .forEach(foo -> kafkaTemplate.send("topic_fail", foo));
            return null;
        });
    }
}
